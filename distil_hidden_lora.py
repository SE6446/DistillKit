...
config = {
    ...
    "lora_hparams": {
        ...
        'dropout': 0,
        'bias': 'none',
        'use_gradient_chackpointing': 'unsloth',
        'random_state': 42,
        'use_rslora':False,
        'loftq_config': None
    }
...
teacher_model, _ = FastLanguageModel.from_pretrained(config["models"]["teacher"], **model_kwargs).to(device)
teacher_model = FastLanguageModel.for_inference(teacher_model)
student_model, _ = FastLanguageModel.from_pretrained(config["models"]["student"], **model_kwargs).to(device)


student_model = student_model.get_peft_model(
    student_model,
    r=config['lora_hparams']['r'],
    target_modules=config['lora_hparams']['target_modules'],
    alpha=config['lora_hparams']['alpha'],
    dropout=config['lora_hparams']['dropout'],
    bias=config['lora_hparams']['bias'],
    use_gradient_checkpointing=config['lora_hparams']['use_gradient_chackpointing'],
    random_state=config['lora_hparams']['random_state'],
    use_rslora=config['lora_hparams']['use_rslora'],
    loftq_config=config['lora_hparams']['loftq_config']
)
